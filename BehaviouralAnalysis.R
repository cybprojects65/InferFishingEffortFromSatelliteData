...
for (i in 1:dim(vessel_behaviours)[1]){
    ...
    if(is.na(vb$mean_mutual_dist))
      vb$mean_mutual_dist<-0
    if(is.na(vb$mean_port_distance))
      vb$mean_port_distance<-0
...
if (as.logical(properties["vessel_behaviour_analysis"])){
  cat(" enabling behaviour filter\n")
  good_indices<-which(vessel_data$mmsi%in%good_vessels)
}else{
  cat(" disabling behaviour filter\n")
  good_indices<-which(vessel_data$mmsi%in%vessel_data$mmsi)
}

cat("percentage of good vessels=",length(good_vessels),"/",length(unique(vessel_behaviours$vessel)),"=",(length(good_vessels)*100.0/length(unique(vessel_behaviours$vessel))),"%\n")
